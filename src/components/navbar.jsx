/**
 * This component renders a navbar with a logo and several links.
 * It uses the useState hook to store the current scroll position and the
 * useEffect hook to add an event listener to the window to detect when the user
 * scrolls and to update the state accordingly.
 * The navbar is hidden when the user scrolls down and shown when the user scrolls up.
 */
...
function Navbar() {
     ...
     * It checks if the user has scrolled down or up and updates the state accordingly.
     */
    const hendleScroll = () => {
        if(window.scrollY > lastScroll){
            ...
            // If the user has scrolled up, show the navbar
            setShowNavbar(false)
    ...
    // useEffect is a hook that runs a function after the component has rendered or when a dependency has changed
    // We use it to set up an event listener and to clean it up when the component is unmounted
    // In this case, we use it to set up a scroll event listener that will hide the navbar when we scroll down
    // and show it when we scroll up
    React.useEffect(() => {
    ...
    return (
        <div id="navbar"
            className={`transform transition-transform duration-700 ${showNavbar ?  'translate-y-[-100%]':'transler-y-0' } fixed backdrop-blur-sm z-[999] w-full px-20 py-5 font-['Bebas Neue'] flex justify-between items-center`}
        >
            <div className="logo">
            <svg
					width="72"
            ...
            <div className="Links flex gap-10">
                {/* Use map to generate multiple similar elements */}
                {["Services", "Our Work", "About Us", "Insights", "Contact Us"].map(
                    (item, index) => (
                        <a
                            key={index}
                            // Use template literals and conditional classes
                            className={`links text-lg capitalize font-normal ${index === 4 && "ml-32"
                                }`}
                        >
                            {item}
                        </a>
